...
val databaseModule = DI.Module("databaseModule") {
    ...
    bind<MoebooruSitesDao>() with singleton { instance<KraftDatabase>().moebooruSitesDao() }
    bind<DanbooruSitesDao>() with singleton { instance<KraftDatabase>().danbooruSitesDao() }
    bind<GelbooruSitesDao>() with singleton { instance<KraftDatabase>().gelbooruSitesDao() }
    bind<SankakuSitesDao>() with singleton { instance<KraftDatabase>().sankakuSitesDao() }
    bind<PixivSitesDao>() with singleton { instance<KraftDatabase>().pixivSitesDao() }
